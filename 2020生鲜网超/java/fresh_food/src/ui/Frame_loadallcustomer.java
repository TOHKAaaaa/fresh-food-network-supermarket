...
public class Frame_loadallcustomer extends JDialog implements ActionListener{
	private JPanel toolBar = new JPanel();
	private Button btnDelete = new Button("删除用户");
	private Object CustomerTitle[]=Bean_customer_infor.tableTitles;
	private Object CustomerData[][];
	DefaultTableModel tablmod=new DefaultTableModel();
	private JTable CustomerTable=new JTable(tablmod);
	private void reloadUserTable(){
		try {
			List<Bean_customer_infor> allCustomer=(new ExampleUserManger()).loadallCustomer();
			CustomerData =new Object[allCustomer.size()][Bean_customer_infor.tableTitles.length];
			for(int i=0;i<allCustomer.size();i++){
				for(int j=0;j<Bean_customer_infor.tableTitles.length;j++)
					CustomerData[i][j]=allCustomer.get(i).getCell(j);
			}
	...
	public Frame_loadallcustomer(Frame f, String s, boolean b) {
		super(f, s, b);
