...
public class Frame_becomeVIP extends JDialog implements ActionListener{
	private JPanel toolBar = new JPanel();
	private JPanel workPane = new JPanel();
	private Button btnOk = new Button("确定");
	private Button btnCancel = new Button("取消");
	
	private JLabel customer_id_label = new JLabel("请输入成为会员月数:");
	private JTextField customer_id_textField = new JTextField();
	private final JLabel lblNewLabel = new JLabel("个月");
	public Frame_becomeVIP(Frame f, String s, boolean b) {
		super(f, s, b);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==this.btnCancel)
			...
			try {
				util.customerManger.becomeVIP(time);
				this.setVisible(false);
			} catch (BaseException e1) {
				JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
			JOptionPane.showMessageDialog(null,"注册成功", "成为会员",JOptionPane.PLAIN_MESSAGE);
			this.setVisible(false);
		}
	}
}
